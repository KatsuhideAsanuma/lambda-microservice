...
use lambda_microservice_controller::{
    api::{
        configure, ExecuteRequest, ExecuteResponse, FunctionInfo, FunctionListResponse,
        FunctionManagerTrait, InitializeRequest, InitializeResponse, RuntimeManagerTrait,
        SessionManagerTrait, SessionStateResponse,
    },
    config::Config,
    error::{Error, Result},
    ...
    runtime::{RuntimeConfig, RuntimeExecuteResponse, RuntimeSelectionStrategy, RuntimeType},
...
impl MockRuntimeManager {
    ...
    pub fn with_execute_in_container_result(
        mut self,
        result: Result<RuntimeExecuteResponse>,
    ) -> Self {
...
async fn test_initialize_session_creation_error() {
    ...
    let app = test::init_service(
        App::new()
            ...
            .app_data(web::Data::new(
                Arc::new(mock_db_logger) as Arc<dyn DatabaseLoggerTrait>
            ))
            .configure(configure),
    )
    .await;
    ...
    assert!(json["error"]
        .as_str()
        .unwrap()
        .contains("Session creation error"));
}
...
async fn test_execute_runtime_error() {
    ...
    let req = test::TestRequest::post()
        ...
        .set_json(&ExecuteRequest { params: json!({}) })
        .to_request();
...
async fn test_execute_session_update_error() {
    ...
    let mock_runtime_manager =
        MockRuntimeManager::new().with_execute_result(Ok(RuntimeExecuteResponse {
