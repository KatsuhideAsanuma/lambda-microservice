...
use lambda_microservice_controller::{
    ...
    runtime::{RuntimeExecuteResponse, RuntimeManager, RuntimeSelectionStrategy, RuntimeType},
...
async fn test_runtime_manager_new() {
    ...
    assert!(runtime_manager
        .get_runtime_type("nodejs-test")
        .await
        .is_ok());
    assert!(runtime_manager
        .get_runtime_type("python-test")
        .await
        .is_ok());
...
async fn test_get_runtime_type() {
    ...
    let runtime_manager = RuntimeManager::new(&config, postgres_pool.clone())
        .await
        .unwrap();
    ...
    assert_eq!(
        runtime_manager
            .get_runtime_type("nodejs-javascript")
            .await
            .unwrap(),
        RuntimeType::NodeJs
    );
    assert_eq!(
        runtime_manager.get_runtime_type("nodejs-js").await.unwrap(),
        RuntimeType::NodeJs
    );
    ...
    assert_eq!(
        runtime_manager.get_runtime_type("rust-test").await.unwrap(),
        RuntimeType::Rust
    );
    assert_eq!(
        runtime_manager.get_runtime_type("rust-app").await.unwrap(),
        RuntimeType::Rust
    );
...
async fn test_compile_with_wasmtime() {
    ...
    let result = runtime_manager
        .compile_with_wasmtime(rust_code, 1024 * 1024)
        .await;
    ...
    match result {
        Ok(wasm_bytes) => {
            assert!(
                !wasm_bytes.is_empty(),
                "Expected non-empty WebAssembly bytes"
            );
            assert_eq!(
                wasm_bytes[0..4],
                [0x00, 0x61, 0x73, 0x6D],
                "Expected WebAssembly magic bytes"
            );
        }
        Err(e) => match e {
            Error::Runtime(msg) => {
...
async fn test_execute_wasm() {
    ...
    if result.is_ok() {
        ...
        println!(
            "Execute WASM error (expected in test environment): {:?}",
            result
        );
        assert!(true);
    }
    ...
    let result = runtime_manager
        .execute_wasm(&session_without_artifact, params)
        .await;
    assert!(result.is_err());
    match result {
        Err(Error::BadRequest(msg)) => {
...
async fn test_execute_in_container() {
    ...
    let result = runtime_manager
        .execute_in_container(RuntimeType::NodeJs, &session, params)
        .await;
